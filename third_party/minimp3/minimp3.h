...
/*
    ...
    Vendored into flash3ds-runtime 2026-08-21 (Roadmap Phase 3, MP3 audio
    decode) for use by src/audio/Mp3Decoder.{h,cpp} — see
    third_party/minimp3/README.md for exact provenance and why this
    (single-header, CC0/public-domain, no dependencies, no dynamic
    allocation beyond the caller-owned decode buffers) is a good fit for a
    codebase that also needs to cross-compile for the Nintendo 3DS, where
    an LGPL library like libmpg123 would need either dynamic linking
    (not how homebrew .3dsx binaries work) or relinkable-object
    compliance to use. `#include <iostream>` (unused by this header — no
    std:: symbol is referenced anywhere in it) was removed from the
    upstream copy this was vendored from; everything else is unmodified.
*/
#include <stdint.h>
