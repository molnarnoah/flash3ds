...
std::vector<uint8_t> buildBitmapFillStyleArrayBytes(uint8_t fillStyleType,
    ...
    std::vector<uint8_t> out;
    writeU8(out, 1);  // count = 1
    writeU8(out, fillStyleType);
    writeU16(out, bitmapCharacterId);
    out.insert(out.end(), bitmapMatrixBytes.begin(), bitmapMatrixBytes.end());
    return out;
}
...
std::vector<uint8_t> buildDefineShapeWithBitmapFillBytes(
    ...
    auto fillStyles =
        buildBitmapFillStyleArrayBytes(fillStyleType, bitmapCharacterId, bitmapMatrixBytes);
    out.insert(out.end(), fillStyles.begin(), fillStyles.end());
    auto lineStyles = buildEmptyLineStyleArrayBytes();
    out.insert(out.end(), lineStyles.begin(), lineStyles.end());
    auto records = buildRectShapeRecordsBytes(widthTwips, heightTwips, false);
    out.insert(out.end(), records.begin(), records.end());
    (void)shapeVersion;  // shape-record bit-packing doesn't vary by version here (mirrors the
                          // gradient builder above, which takes the same unused-looking param
                          // for symmetry/future-proofing rather than actually branching on it).
    return out;
}
...
// zlib-compresses `raw` at max compression — shared by every
// DefineBitsLossless/2/Jpeg3-alpha fixture builder below. Mirrors
// wrapCws()'s own compress2() call above, factored out since three
// different builders need it (wrapCws() itself stays a one-off, not worth
// retrofitting onto this).
std::vector<uint8_t> zlibCompressBytes(const std::vector<uint8_t>& raw) {
    uLongf bound = compressBound(static_cast<uLong>(raw.size()));
    std::vector<uint8_t> compressed(bound);
    uLongf compressedSize = bound;
    int ret = compress2(compressed.data(), &compressedSize,
                          raw.empty() ? reinterpret_cast<const Bytef*>("") : raw.data(),
                          static_cast<uLong>(raw.size()), Z_BEST_COMPRESSION);
    compressed.resize(ret == Z_OK ? compressedSize : 0);
    return compressed;
}

// Pads `bytes` up to the next multiple of 4 with zero bytes — the SWF
// spec's "every DefineBitsLossless/2 scanline is padded to a 32-bit
// boundary" rule, applied identically by every colormapped/15-bit/24-32-
// bit row this file's fixture builders emit.
void padRowTo4(std::vector<uint8_t>& bytes) {
...
std::vector<uint8_t> buildDefineBitsLosslessRgbBytes(
    ...
    for (const auto& row : rows) {
        ...
        // PIX24 is already 4 bytes/pixel, so width*4 is always a multiple
        // of 4 — padRowTo4() is a no-op here, called anyway for symmetry
        // with the other two builders below (and in case a future variant
        // needs a non-4-aligned width*bytesPerPixel).
        padRowTo4(raw);
    }
    std::vector<uint8_t> compressed = zlibCompressBytes(raw);

    std::vector<uint8_t> out;
    writeU16(out, characterId);
    writeU8(out, 5);  // BitmapFormat: 24-bit RGB (PIX24)
    writeU16(out, width);
    writeU16(out, height);
    out.insert(out.end(), compressed.begin(), compressed.end());
    return out;
...
std::vector<uint8_t> buildDefineBitsLossless2ArgbBytes(
    ...
    for (const auto& row : rows) {
        for (const auto& px : row) {
            // Premultiply, matching the real ARGB32 encoding — see this
            // function's own declaration comment in SwfTestFixtures.h.
            auto premul = [a = px.a](uint8_t c) -> uint8_t {
                return static_cast<uint8_t>((static_cast<int>(c) * a + 127) / 255);
            };
            raw.push_back(px.a);
            raw.push_back(premul(px.r));
            raw.push_back(premul(px.g));
            raw.push_back(premul(px.b));
        }
        padRowTo4(raw);  // no-op (4 bytes/pixel), see buildDefineBitsLosslessRgbBytes above
    }
    std::vector<uint8_t> compressed = zlibCompressBytes(raw);

    std::vector<uint8_t> out;
    writeU16(out, characterId);
    writeU8(out, 5);  // BitmapFormat: 32-bit ARGB
    writeU16(out, width);
    writeU16(out, height);
    out.insert(out.end(), compressed.begin(), compressed.end());
    return out;
...
std::vector<uint8_t> buildDefineBitsLosslessColormapBytes(
    ...
    for (const auto& entry : colorTable) {
        raw.push_back(entry.r);
        raw.push_back(entry.g);
        raw.push_back(entry.b);
        if (version2) raw.push_back(entry.a);
    }
    ...
    std::vector<uint8_t> out;
    writeU16(out, characterId);
    writeU8(out, 3);  // BitmapFormat: 8-bit colormapped
    writeU16(out, width);
    writeU16(out, height);
    writeU8(out, static_cast<uint8_t>(colorTable.size() - 1));  // BitmapColorTableSize
    out.insert(out.end(), compressed.begin(), compressed.end());
    return out;
}

const std::vector<uint8_t>& sampleTinyJpegBytes() {
    // Generated 2026-08-31 via:
    //   from PIL import Image
    //   im = Image.new('RGB', (8, 8))
    //   px = im.load()
    //   for y in range(8):
    //       for x in range(8):
    //           px[x, y] = (200, 20, 20) if x < 4 else (20, 20, 200)
    //   im.save('test_tiny.jpg', format='JPEG', quality=100, subsampling=0)
    // 670 bytes: a real baseline JPEG (8x8, left half dark red, right half
    // dark blue) — see this array's declaration in SwfTestFixtures.h for
    // why an external encoder's output is used here rather than hand-built
    // bytes, same rationale as sampleMp3AudioBytes() above.
    static const std::vector<uint8_t> kBytes = {
        ...
        0x72, 0xfe, 0x3c, 0x7f, 0xcc, 0xab, 0xff, 0x00, 0x71, 0xcf, 0xfd, 0xc3, 0xd7, 0xf7, 0x87, 0xec,
        0x4c, 0xff, 0x00, 0x9c, 0x99, 0xff, 0x00, 0xbc, 0x33, 0xff, 0x00, 0xc1, 0x5c, 0xff, 0x00, 0x4b,
        0x3f, 0x6b, 0xef, 0xfc, 0xe3, 0xcf, 0xfd, 0xe5, 0x9f, 0xfe, 0x06, 0x67, 0xff, 0xd9,
    };
    return kBytes;
}
...
std::vector<uint8_t> buildDefineBitsJpeg2Bytes(uint16_t characterId,
    ...
    std::vector<uint8_t> out;
    writeU16(out, characterId);
    out.insert(out.end(), jpegBytes.begin(), jpegBytes.end());
    return out;
}
...
std::vector<uint8_t> buildDefineBitsJpeg3Bytes(uint16_t characterId,
    ...
    std::vector<uint8_t> out;
    writeU16(out, characterId);
    writeU32(out, static_cast<uint32_t>(jpegBytes.size()));
    out.insert(out.end(), jpegBytes.begin(), jpegBytes.end());
    if (!alphaBytes.empty()) {
        std::vector<uint8_t> compressed = zlibCompressBytes(alphaBytes);
        out.insert(out.end(), compressed.begin(), compressed.end());
    }
    return out;
}
