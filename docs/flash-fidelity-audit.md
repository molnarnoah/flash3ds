...
1. **`Sound.setVolume()` is a complete no-op on the 3DS backend — HIGH impact, LOW-MODERATE fix effort.** `IAudioBackend` has no volume method whatsoever, so any AS2 `Sound.setVolume(n)` call currently has zero audible effect. This directly matters for the exact corpus under test: `hobo.swf`'s own frame-1 mute button calls `setVolume(0)`/`setVolume(100)` on every End-key press (confirmed via `hobo_frame_progression_probe`'s trace this session) — once sound is actually reachable, that mute toggle will do nothing audible. **Fix:** add `setVolume(uint16_t soundId, float volume)` to `IAudioBackend`, thread it into the `ndspChnSetMix` mix array (`mix[0]=mix[1]=volume` instead of the hardcoded `1.0f`) in `Nintendo3DSAudioBackend::playSound()`, and wire AS2 `Sound.setVolume`'s native impl (currently presumably a stored-but-unused property — verify) to call it.
2. **Channel allocation has no eviction policy — MODERATE impact, LOW fix effort.** All 23 usable channels can be permanently consumed by one-shot sounds that never call `stopSound()`, silently dropping later sounds once exhausted (a `LOG_WARN`, no crash). `hobo.swf` has 266 real `StartSound` triggers across many sprites — plausible to hit in busy gameplay once reachable. **Fix:** track each channel's `ndspChnIsPlaying()` state and treat a finished one-shot channel as free again without requiring an explicit `stopSound()`.
3. **No counted `StartSound` loop repeat — MODERATE impact, MODERATE fix effort** (already documented, not attempted): `loopCount > 1` always plays once. **Fix:** queue N chained `ndspWaveBuf`s pointing at the same PCM (the approach the code's own comment already proposes), needs on-device verification.
4. **`SyncNoMultiple` is parsed but never consulted — LOW-MODERATE impact.** A StartSound flagged "don't restart if already playing" will still re-trigger every time its frame is re-entered (e.g. a looping timeline), causing overlapping/duplicate playback Flash Player would have suppressed. **Fix:** before calling `playSoundById()` in `runCurrentFrameSounds()`, check `event.info.syncNoMultiple` against whether that soundId's channel is currently playing (needs a `isPlaying(soundId)` query added to `IAudioBackend`).
5. **`SoundStreamHead`/`SoundStreamBlock` entirely unimplemented — LOW impact for this specific corpus (confirmed zero real stream data in `hobo.swf`), but likely HIGH impact for any title that uses real streaming background music.** No parser, no dispatch case exists at all. **Fix:** this is a real, non-trivial feature addition (frame-synced streaming PCM decode + queuing), not a quick patch — scope it separately if a target title is confirmed to need it.
6. **Non-MP3 codecs (ADPCM/Nellymoser/Speex/uncompressed) never decoded — impact unknown, corpus-dependent.** Structurally parsed, `ensureSoundDecoded()` explicitly early-returns for anything but `kMp3`. **Fix:** only worth doing if a target title's corpus is shown to need a specific one of these (same reasoning this project already applied to MP3-only support).
7. **SOUNDINFO envelope + in/out points parsed but never applied — LOW impact.** Per-sample volume automation and sound trimming are silently ignored. Rare in practice for this corpus's likely SFX-style usage; low priority.
